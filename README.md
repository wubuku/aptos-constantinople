...
* [owned_monsters_add_monster_logic.move](./aptos-contracts/sources/owned_monsters_add_monster_logic.move). This is an example of defining a method on an aggregate (entity). We don't want to generate and use the UPDATE method directly. We want to have better semantics for methods that operate on individual entities.
* [rpg_service.move](./aptos-contracts/sources/rpg_service.move). The implementation of the domain service. The signature part of the public entry fun here is already generated by the tool, we just need to implement the function body.
