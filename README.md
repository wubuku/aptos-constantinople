...
* `enableMultipleMoveProjects` means to generate multiple Move contract projects. Currently, projects are generated based on the DDDML modules declared in the model (note that we are here talking about modules in the DDD sense, not Move modules.) Aptos has a limit on the size of packages that can be published, which can't exceed 60k.
...
* [map_service.move](aptos-contracts/constantinople-map/sources/map_service.move). Here the map initialization logic is implemented. The signature part of the public entry fun here is already generated by the tool, we just need to implement the function body.
* [rpg_service.move](aptos-contracts/constantinople/sources/rpg_service.move). The implementation of the main domain service. The signature part of the public entry fun here is already generated by the tool, we just need to implement the function body.
